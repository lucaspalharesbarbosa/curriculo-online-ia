"""Cliente de busca web (Tavily) — enriquecimento opcional do RAG (ADR-010 seção 2).

Só é chamado por `chat.py` quando a busca local (`rag.search_with_routing`)
não encontrou contexto suficiente e a pergunta cita uma entidade conhecida do
currículo. Nunca propaga exceção: qualquer falha (timeout, erro de rede,
resposta inesperada do provedor) vira `None`, e o chamador cai no fallback
local — a busca web é só enriquecimento, nunca pode derrubar o `/chat`.
"""
...
def search_web(query: str) -> str | None:
    ...
    `None` cobre: sem `WEB_SEARCH_API_KEY` configurada, timeout, erro HTTP/rede
    do provedor, ou resposta sem conteúdo aproveitável — em todos os casos o
    chamador deve seguir para o fallback local, sem levantar erro ao client.
    """

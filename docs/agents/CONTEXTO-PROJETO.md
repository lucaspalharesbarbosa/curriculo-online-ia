...
| Fase 14 — Arquitetura & Modularização | Reorganização de backend/frontend por domínio (`resume`/`chat`, preparado para `admin`), DDD-lite; Ports & Adapters (Clean Architecture seletiva) no domínio `chat` | `docs/product/backlog/fase-14/` (US-14-01 a US-14-04) — US-14-01 Done; US-14-02 aguardando validação de preview; US-14-03 implementada (aguardando QA/Tech Lead); US-14-04 Ready for Agent |

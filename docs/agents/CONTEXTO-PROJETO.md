...
| 013 | Memória Conversacional (RAG) |
...
| Fase 15 — Memória Conversacional (RAG) | `ChatRequest.history` opcional (janela deslizante), reformulação da pergunta antes do retrieval, histórico no prompt final, frontend envia o histórico já mantido em `useResumeChat` — backend continua sem sessão persistida em servidor | `docs/product/backlog/fase-15/` (US-15-01 a US-15-03) — US-15-01 (`ADR-014`) e US-15-02 (backend) `Done`; US-15-03 (frontend) Dev/QA/Tech Lead aprovados, aguardando push/PR/preview/aceite do PO; `feature/chat-memoria-conversacional` |

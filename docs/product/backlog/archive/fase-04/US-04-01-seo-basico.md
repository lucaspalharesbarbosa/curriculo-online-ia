...
**Status:** Done — CA-001/002 fechados e DoD 100% fechado em 2026-08-04. Achados Low do Tech Lead **resolvidos em 2026-08-04**: `title` agora usa só o primeiro cargo de `hero.title` (antes do `|`) — `"Lucas Palhares Barbosa — Tech Lead"`, 35 chars, bem abaixo do limite de ~60; `siteUrl` passa a aceitar override via `NEXT_PUBLIC_SITE_URL` (fallback mantém o valor documentado em `frontend/README.md`), reduzindo o risco de dessincronia. Build/lint/testes revalidados sem regressão

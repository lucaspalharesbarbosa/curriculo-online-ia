...
**Status:** Done — CA-001/002/003 fechados e DoD 100% fechado em 2026-08-04. Follow-up não bloqueante **resolvido em 2026-08-15**: auditoria Lighthouse (categoria Acessibilidade) rodada contra produção (`https://lucas-palhares-cv.vercel.app`) com o site já com todas as seções (pós Fase 07) — score 96/100, 2 achados, ambos fora do escopo original desta história (introduzidos pelo `MobileBottomNav`, componente da Fase 07): contraste insuficiente (`text-neutral-500` sobre fundo escuro, 4.21:1 < 4.5:1 exigido) e nome acessível divergente do texto visível em 2 abas ("Certs"/"Certificações", "Projetos"/"Destaques"). Corrigidos em `frontend/components/MobileBottomNav.tsx` (`text-neutral-500` → `text-neutral-400`) e `frontend/lib/mobile-nav.ts` (`short: "Certs"` → `"Cert."`, `short: "Projetos"` → `"Destaques"`, alinhando com o título real da seção em `ProjectsSection.tsx`). Reauditoria local (build de produção) confirma **100/100**. Lint e testes (`MobileBottomNav.test.tsx`) sem regressão. Correção pontual fora do fluxo formal de pipeline (achado de auditoria, não US nova) — sem commit ainda, a critério do autor.

...
**Status:** Done — CA-001/002/003/004 fechados e DoD 100% fechado em 2026-08-04 (revalidação pós ação humana no painel Vercel: GitHub conectado, Root Directory/Production Branch ajustados). URL de produção na data do Done: https://curriculo-online-ia.vercel.app (renomeada em 2026-08-10 para `lucas-palhares-cv.vercel.app`). Pendência não bloqueante **resolvida em 2026-08-15**: domínio extra `curriculo-online-ia.vercel.app` removido pelo autor no painel Domains da Vercel, restando só `lucas-palhares-cv.vercel.app` como produção. Verificado pós-remoção: `lucas-palhares-cv.vercel.app` → HTTP 200; `curriculo-online-ia.vercel.app` → HTTP 404 (limpo, sem afetar o domínio ativo); backend (Render) → HTTP 200; preflight CORS do `/chat` já respondia `access-control-allow-origin: https://lucas-palhares-cv.vercel.app` antes e depois da remoção — nenhuma dependência de código no domínio removido.

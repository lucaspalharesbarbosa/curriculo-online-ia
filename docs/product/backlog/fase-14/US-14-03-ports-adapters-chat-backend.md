...
| PO | `@product-owner` | Quase lá — verificação independente confirma o que QA e Tech Lead reportaram: `pytest -q` 77/77 verdes, `pytest --cov=app --cov-report=term-missing` 98% total (`ports.py`/`service.py`/`router.py`/`openai_adapter.py` 100%, `rag.py` 96%, `tavily_adapter.py` 97%), acima do piso de 70%; `ruff check .` e `black --check .` limpos. Todos os CA `[x]`, tasks `[x]`. Falta fechar só um item de DoD: "Deploy/preview verificado" segue `[ ]` — smoke real de `/chat`/`/chat/feedback` com `LLM_API_KEY` válida depende de merge `develop`→`main` (Render só faz deploy a partir de `main`, `render.yaml`), decisão do autor sobre quando mergear para produção. Branch ainda sem PR aberto para `develop` — abrir antes do merge | 2026-08-19 | `pytest -q`, `pytest --cov=app --cov-report=term-missing`, `ruff check .`, `black --check .` (execução independente nesta sessão) |

**Status:** Quase lá — Dev/QA/Tech Lead aprovados; pendente abrir PR para `develop`, mergear `develop`→`main` e validar `/chat`/`/chat/feedback` em produção antes de fechar Done

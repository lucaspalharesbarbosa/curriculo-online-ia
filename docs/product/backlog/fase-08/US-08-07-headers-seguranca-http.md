...
- [ ] CA-001: `curl -I` no frontend em produção retorna `Content-Security-Policy`, `X-Content-Type-Options: nosniff`, `X-Frame-Options: DENY` (ou `frame-ancestors 'none'` na própria CSP), `Referrer-Policy` e `Permissions-Policy` — **implementado e validado localmente** (`npx next start` real + `curl -I` confirma os 5 headers, ver Vereditos QA); falta apenas o `curl -I` contra a URL de produção real na Vercel — **pendente de ação humana**: agente não tem acesso a produção real
- [ ] CA-002: `curl -I` no backend em produção (`/health` e/ou `/chat`) retorna os mesmos headers, adicionados via middleware FastAPI — **implementado e validado localmente** (`TestClient` real em `/health` confirma os 5 headers, teste automatizado + verificação manual, ver Vereditos QA); falta apenas o `curl -I` contra a URL de produção real no Render — **pendente de ação humana**: agente não tem acesso a produção real
- [ ] CA-003: após o deploy, fontes, imagens e o fluxo do `ChatWidget` continuam funcionando normalmente — sem erro de CSP bloqueando recurso no console do navegador (smoke manual documentado no PR/QA) — **pendente de ação humana**: exige navegação real no site em produção (Vercel), fora do alcance do agente; análise estática do código (fontes via `next/font/google` self-hospedadas, imagens sempre locais em `public/`, chat same-origin via `/api/chat`) indica que a CSP configurada não deveria bloquear nada, mas isso não substitui o smoke real — **confirmado**: o smoke real feito pelo autor encontrou exatamente o gap que a análise estática não cobria (scripts inline injetados pelo próprio Next.js para hidratação), site ficou em branco em produção; causa raiz e correção em [US-08-09](US-08-09-fix-csp-bloqueava-hidratacao.md)
...
- [ ] T04 Smoke manual pós-deploy: `curl -I` em produção (frontend e backend) + navegação verificando fontes/imagens/chat sem erro de CSP no console — **pendente**: ação manual em produção real (Vercel + Render), sem acesso do agente
...
- [ ] Todos os critérios de aceite acima `[x]` — CA-003 aberto; CA-001/CA-002 abertos só na perna de produção real (ver justificativa em cada CA)
...
- [ ] Deploy/preview verificado — `curl -I` real em produção (frontend Vercel + backend Render) e navegação manual sem erro de CSP — **pendente de ação humana**: agente não tem acesso a produção real; validado localmente com evidência equivalente (`npx next start` + `curl -I`, `TestClient` real no backend)
...
| PO | `@product-owner` | Quase lá | 2026-08-15 | CA-004 e CA-005 genuinamente fechados, com evidência real (código + teste automatizado + `git diff --stat`). CA-001 e CA-002 têm a implementação completa e validada localmente com evidência equivalente à de produção (build de produção real + `curl -I` no frontend; `TestClient` real no backend), mas ficam `[ ]` porque a história pede especificamente `curl -I` **em produção** — não forço Done nesse ponto, mesmo com a implementação correta. CA-003 fica `[ ]` — depende de navegação manual real no site em produção, fora do alcance do agente; a análise estática (fontes self-hospedadas, imagens locais, chat same-origin) é evidência de apoio, não substitui o smoke. T04 e o item de DoD "Deploy/preview verificado" ficam pendentes pela mesma razão. Restante do DoD fechado. Falta, para Done: (1) deploy do frontend na Vercel e do backend no Render com este código; (2) `curl -I` na URL de produção de cada serviço confirmando os 5 headers; (3) navegação manual no site publicado checando console do navegador sem erro de CSP ao carregar fontes, imagens e usar o `ChatWidget`; (4) marcar CA-001, CA-002, CA-003, T04 e o item de DoD correspondente como concluídos com essa evidência. |

...
- [ ] CA-003: após o deploy, fontes, imagens e o fluxo do `ChatWidget` continuam funcionando normalmente — sem erro de CSP bloqueando recurso no console do navegador (smoke manual documentado no PR/QA) — **pendente de ação humana**: exige navegação real no site em produção (Vercel), fora do alcance do agente; análise estática do código (fontes via `next/font/google` self-hospedadas, imagens sempre locais em `public/`, chat same-origin via `/api/chat`) indica que a CSP configurada não deveria bloquear nada, mas isso não substitui o smoke real

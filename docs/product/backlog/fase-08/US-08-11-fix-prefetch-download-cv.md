...
- [x] CA-003: Lighthouse mobile em produção real, pós-deploy, não reporta nenhum item de `errors-in-console` relacionado ao arquivo PDF do currículo — confirmado em 2026-08-16, pós-merge de `develop`→`main` (PR #44): `npx lighthouse https://lucas-palhares-cv.vercel.app --chrome-flags="--headless=new --no-sandbox --disable-gpu"` (Chrome local, `lighthouse-mobile.report.json`) → `errors-in-console` score **1**, `details.items: []` (nenhum erro de console, mobile e desktop)
- [x] CA-004: Boas Práticas do Lighthouse mobile em produção chega a 100 (paridade com desktop, que já estava em 100 em `QA-006`) — confirmado no mesmo relatório: Boas Práticas mobile **100** (era 96 em `QA-006`), Acessibilidade 100, SEO 100, Performance 92
...
- [x] T04 Lighthouse mobile + desktop em produção pós-deploy confirmando CA-003/CA-004 — feito em 2026-08-16, ver evidência em CA-003/CA-004
...
- [x] Deploy/preview verificado — merge `develop`→`main` (PR #44) em 2026-08-16, Vercel deployou automaticamente; Lighthouse real em produção confirma CA-003/CA-004; `curl` confirma `<a href="/Lucas_Palhares_Barbosa_Engenheiro_De_Software.pdf" download="">` puro no HTML servido e PDF respondendo 200
...
| PO | `@product-owner` | **Aceite (Done)** — CA-001 a CA-005 fechados com evidência real. Após merge `develop`→`main` (PR #44) e deploy automático da Vercel, rodei Lighthouse mobile+desktop real em produção: `errors-in-console` limpo (score 1, 0 itens) e Boas Práticas mobile subiu de 96 (`QA-006`) para **100** — confirma que a causa raiz (prefetch RSC do `next/link` no PDF) foi eliminada. `curl` confirma `<a download>` nativo servido de verdade. DoD 100% fechado | 2026-08-16 | `lighthouse-mobile.report.json`/`lighthouse-desktop.report.json` (gerados localmente, não commitados — mesmo padrão de `QA-006`) |

**Status:** Done — CA-001 a CA-005 fechados com evidência real coletada em produção pós-deploy (PR #44, 2026-08-16).

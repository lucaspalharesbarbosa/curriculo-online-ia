...
/**
 * Proxy same-origin do feedback do chat (US-11-04, mesmo padrão do `/api/chat`).
 * O browser chama `/api/chat/feedback`; o Next encaminha ao FastAPI — sem CORS no client.
 */
export async function POST(request: Request) {
  ...
  try {
    const upstream = await fetch(`${API_URL}/chat/feedback`, {
      method: "POST",
